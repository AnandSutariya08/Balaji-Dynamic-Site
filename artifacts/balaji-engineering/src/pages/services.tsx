...
import {
  ArrowRight, CheckCircle2, Zap, Shield, Clock, Layers,
...
const services = [
  {
    ...
    specs: [
      { label: "Max Tonnage", value: "800T" },
      { label: "Max Length", value: "6000mm" },
      { label: "Accuracy", value: "±0.5°" },
      { label: "Min Radius", value: "0.5T" },
    ],
    color: "#AC3C3C"
  },
  {
    ...
    specs: [
      { label: "Min Thickness", value: "0.5mm" },
      { label: "Max Thickness", value: "25mm" },
      { label: "Batch Size", value: "1 – 10,000+" },
      { label: "Lead Time", value: "24–72 hrs" },
    ],
    color: "#1a3a6a"
  },
  {
    ...
    specs: [
      { label: "Max Thickness", value: "60mm MS" },
      { label: "Max Sheet Size", value: "3000 × 12000mm" },
      { label: "Squareness", value: "±0.3mm" },
      { label: "Surface", value: "Burr-Free" },
    ],
    color: "#AC3C3C"
  },
  {
    ...
    specs: [
      { label: "Max Plate Width", value: "3000mm" },
      { label: "Max Thickness", value: "60mm" },
      { label: "Min Diameter", value: "300mm" },
      { label: "Ovality", value: "<1%" },
    ],
    color: "#1a3a6a"
  },
  {
    ...
    specs: [
      ...
      { label: "Max Thickness", value: "25mm MS / 15mm SS" },
      { label: "Bed Size", value: "3000 × 6000mm" },
      { label: "Edge Quality", value: "Ra 3.2μm" },
    ],
    color: "#AC3C3C"
  },
  {
    ...
    specs: [
      { label: "Max Size", value: "2000 × 2000mm" },
      { label: "Max Thickness", value: "100mm" },
      { label: "Hole Tolerance", value: "H7" },
      { label: "Flatness", value: "±0.3mm/m" },
    ],
    color: "#1a3a6a"
  },
  {
    ...
    specs: [
      { label: "Max Thickness", value: "150mm (Oxy)" },
      { label: "Plasma Thickness", value: "60mm" },
      { label: "Positional Acc.", value: "±0.5mm" },
      { label: "Min Feature", value: "5mm" },
    ],
    color: "#AC3C3C"
  },
  {
    ...
    description: "High-volume sheet metal shearing services for blanks, strips, and custom-size cuts. Our hydraulic guillotine shears deliver consistent squareness across large production batches, while advanced nesting software minimizes material waste and reduces your per-unit cost.",
    ...
    specs: [
      ...
      { label: "Max Thickness", value: "16mm" },
      { label: "Squareness", value: "±0.2mm" },
      { label: "Min Batch", value: "1 piece" },
    ],
    color: "#1a3a6a"
  }
];
  ...
  return (
    <PageTransition>
      <div className="bg-black">
          ...
          <div className="container relative z-10 mx-auto px-4 pt-32 pb-24">
            <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9 }} className="max-w-4xl">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/20 border border-primary/40 text-primary mb-10">
                <span className="text-[10px] font-bold tracking-[0.3em] uppercase">8 Core Capabilities · Heavy Fabrication · Gujarat</span>
              </div>
              ...
              <p className="text-xl text-zinc-400 font-light leading-relaxed max-w-2xl">
                Advanced CNC machinery. 20 years of expertise. End-to-end fabrication from raw plate to finished component — all under one roof in Surat, Gujarat.
              </p>
            </motion.div>
          </div>
        </section>
        ...
        <section className="py-6 bg-primary overflow-hidden">
          <div className="flex whitespace-nowrap" style={{ animation: "marquee 25s linear infinite" }}>
            {Array.from({ length: 3 }).map((_, i) => (
              <div key={i} className="flex items-center gap-6 px-4 text-sm font-bold text-white/80 uppercase tracking-widest">
        ...
        <section className="py-8 bg-black">
          {services.map((service, i) => (
            <motion.div
              ...
              className={`grid lg:grid-cols-2 border-b border-white/5 group ${i % 2 === 0 ? '' : 'direction-rtl'}`}
            >
              {/* Image */}
              <div className={`relative h-[500px] overflow-hidden ${i % 2 !== 0 ? 'lg:order-2' : ''}`}>
                <img
                  src={service.image}
                  alt={service.title}
                ...
                {/* Index number */}
                <div className="absolute top-8 left-8 text-[5rem] font-display font-black text-white/5 leading-none">
                  {String(i + 1).padStart(2, "0")}
                </div>
                {/* Specs overlay */}
                <div className="absolute bottom-0 left-0 right-0 p-8 grid grid-cols-4 gap-3">
                  {service.specs.map((spec, j) => (
                    <div key={j} className="bg-black/70 backdrop-blur-sm border border-white/10 rounded-xl p-3 text-center">
                      <div className="text-lg font-display font-black text-primary">{spec.value}</div>
                      <div className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest mt-1">{spec.label}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {/* Content */}
              <div className={`p-12 xl:p-16 flex flex-col justify-center bg-[#050505] ${i % 2 !== 0 ? 'lg:order-1' : ''}`}>
                <span className="text-primary font-bold tracking-[0.3em] uppercase text-xs mb-4">{service.tagline}</span>
                <h2 className="text-4xl xl:text-5xl font-display font-black text-white uppercase tracking-tighter mb-6 leading-[0.9]">
                  {service.title}
                </h2>
                <p className="text-zinc-400 font-light leading-relaxed mb-8 text-base">{service.description}</p>
                ...
                <div className="grid grid-cols-2 gap-3 mb-10">
                  {service.features.map((f, j) => (
                    <div key={j} className="flex items-center gap-2.5">
                      ...
                      <span className="text-xs font-bold text-zinc-300 uppercase tracking-wider">{f}</span>
                    </div>
                  ))}
                </div>

                <Button
                  className="w-fit h-14 px-8 font-bold uppercase tracking-widest bg-primary hover:bg-primary/90 text-white shadow-[0_0_20px_rgba(172,60,60,0.3)] border-none"
                  asChild
                >
        ...
        <section className="py-32 bg-[#040404] border-t border-white/5">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-px bg-white/5">
              {capabilities.map((cap, i) => (
                <motion.div
                  ...
                  className="p-10 bg-black hover:bg-zinc-900/50 transition-colors group"
                >
                  <div className="text-5xl font-display font-black text-primary/20 mb-4 group-hover:text-primary/40 transition-colors">
                    {String(i + 1).padStart(2, "0")}
                  </div>
                  <h4 className="text-xl font-display font-black text-white uppercase tracking-tight mb-2">{cap.label}</h4>
                  <p className="text-sm text-zinc-500 font-light">{cap.value}</p>
                </motion.div>
              ))}
            </div>
          </div>
            ...
            <div className="grid lg:grid-cols-2 gap-24 items-center">
              <div>
                ...
                <p className="text-zinc-400 text-lg font-light leading-relaxed mb-12">
                  We work with a comprehensive range of ferrous metals and structural steels, covering the full spectrum of industrial fabrication needs.
                </p>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="bg-zinc-900/80">
                      <th className="px-6 py-5 text-xs font-bold uppercase tracking-widest text-zinc-500">Material</th>
                      <th className="px-6 py-5 text-xs font-bold uppercase tracking-widest text-zinc-500">Grades</th>
                      <th className="px-6 py-5 text-xs font-bold uppercase tracking-widest text-zinc-500">Thickness</th>
                    </tr>
                  </thead>
                  <tbody>
                    {materials.map((m, i) => (
                      <tr key={i} className="border-t border-white/5 hover:bg-zinc-900/30 transition-colors group">
                        <td className="px-6 py-5 font-bold text-white text-sm group-hover:text-primary transition-colors">{m.name}</td>
                        <td className="px-6 py-5 text-zinc-500 text-xs font-mono">{m.grades}</td>
                        <td className="px-6 py-5 text-zinc-400 text-xs font-bold">{m.thickness}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
              {[
                ...
                <motion.div
                  ...
                  className="p-8 border border-white/5 rounded-2xl group hover:border-primary/30 hover:bg-primary/5 transition-all"
                >
          ...
          <div className="container relative z-10 mx-auto px-4 text-center">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8 }}>
              ...
              <p className="text-xl text-zinc-400 font-light max-w-xl mx-auto mb-14">
                Upload your drawings or describe your requirements. We respond within hours.
              </p>
              <div className="flex flex-col sm:flex-row items-center justify-center gap-6">
                ...
                <a href="tel:+917942957640" className="text-2xl font-display font-black text-white hover:text-primary transition-colors">
                  +91-7942957640
                </a>
              </div>
            </motion.div>
