  ...
  return (
    <PageTransition>
      <div className="bg-[#F7F5F1]">
        ...
        <section className="relative min-h-[80vh] flex items-center overflow-hidden">
          ...
          <div className="container relative z-10 mx-auto px-4 pt-32 pb-24">
            <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9 }} className="max-w-4xl">
              ...
              <p className="text-xl text-zinc-300 font-light leading-relaxed max-w-2xl">Advanced CNC machinery. 20 years of expertise. End-to-end fabrication from raw plate to finished component — all under one roof in Surat, Gujarat.</p>
            </motion.div>
          </div>
        </section>
          ...
          {services.map((service, i) => (
            ...
            >
              ...
              <div className={`relative h-[500px] overflow-hidden ${i % 2 !== 0 ? 'lg:order-2' : ''}`}>
                ...
                <div className="absolute top-8 left-8 text-[5rem] font-display font-black text-white/5 leading-none">{String(i + 1).padStart(2, "0")}</div>
                <div className="absolute bottom-0 left-0 right-0 p-8 grid grid-cols-4 gap-3">
                  {service.specs.map((spec, j) => (
                    <div key={j} className="bg-black/60 backdrop-blur-sm border border-white/10 rounded-xl p-3 text-center">
                      <div className="text-lg font-display font-black text-primary">{spec.value}</div>
                      <div className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest mt-1">{spec.label}</div>
                    </div>
                  ))}
                </div>
              </div>
              {/* Content */}
              <div className={`p-12 xl:p-16 flex flex-col justify-center bg-[#F7F5F1] ${i % 2 !== 0 ? 'lg:order-1' : ''}`}>
                <span className="text-primary font-bold tracking-[0.3em] uppercase text-xs mb-4">{service.tagline}</span>
                <h2 className="text-4xl xl:text-5xl font-display font-black text-[#1A1A1A] uppercase tracking-tighter mb-6 leading-[0.9]">{service.title}</h2>
                <p className="text-slate-600 font-light leading-relaxed mb-8 text-base">{service.description}</p>
                ...
                <Button className="w-fit h-14 px-8 font-bold uppercase tracking-widest bg-primary hover:bg-primary/90 text-white shadow-[0_0_20px_rgba(172,60,60,0.25)] border-none" asChild>
        ...
        <section className="py-32 bg-[#EDEAE4] border-t border-black/8">
          <div className="container mx-auto px-4">
            <div className="text-center mb-20">
              <span className="text-primary font-bold tracking-[0.3em] uppercase text-sm">Infrastructure</span>
              <h2 className="text-5xl md:text-7xl font-display font-black text-[#1A1A1A] uppercase tracking-tighter mt-4">Our Machinery</h2>
            </div>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-px bg-black/8">
              {capabilities.map((cap, i) => (
                <motion.div key={i} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ delay: i * 0.08 }} className="p-10 bg-[#EDEAE4] hover:bg-[#F7F5F1] transition-colors group">
                  <div className="text-5xl font-display font-black text-primary/20 mb-4 group-hover:text-primary/40 transition-colors">{String(i + 1).padStart(2, "0")}</div>
                  <h4 className="text-xl font-display font-black text-[#1A1A1A] uppercase tracking-tight mb-2">{cap.label}</h4>
          ...
          <div className="container mx-auto px-4">
            <div className="grid lg:grid-cols-2 gap-24 items-center">
              <div>
                ...
                <h2 className="text-5xl md:text-7xl font-display font-black text-[#1A1A1A] uppercase tracking-tighter mt-4 mb-8 leading-[0.9]">What We<br />Process</h2>
                <p className="text-slate-600 text-lg font-light leading-relaxed mb-12">We work with a comprehensive range of ferrous metals and structural steels, covering the full spectrum of industrial fabrication needs.</p>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="bg-[#EDEAE4]">
                      <th className="px-6 py-5 text-xs font-bold uppercase tracking-widest text-slate-500">Material</th>
                      <th className="px-6 py-5 text-xs font-bold uppercase tracking-widest text-slate-500">Grades</th>
                      <th className="px-6 py-5 text-xs font-bold uppercase tracking-widest text-slate-500">Thickness</th>
                    </tr>
                  </thead>
                  <tbody>
                    {materials.map((m, i) => (
                      <tr key={i} className="border-t border-black/8 hover:bg-[#EDEAE4] transition-colors group">
                        <td className="px-6 py-5 font-bold text-[#1A1A1A] text-sm group-hover:text-primary transition-colors">{m.name}</td>
                        <td className="px-6 py-5 text-slate-500 text-xs font-mono">{m.grades}</td>
                        <td className="px-6 py-5 text-slate-600 text-xs font-bold">{m.thickness}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        {/* WHY CHOOSE US */}
        <section className="py-28 bg-[#EDEAE4]">
          <div className="container mx-auto px-4">
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
              {[
                ...
                <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className="p-8 border border-black/8 rounded-2xl group hover:border-primary/40 hover:bg-primary/5 transition-all bg-[#F7F5F1]">
        ...
        <section className="py-40 bg-[#1C1C1C] relative overflow-hidden">
          ...
          <div className="container relative z-10 mx-auto px-4 text-center">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8 }}>
              <h2 className="text-6xl md:text-8xl font-display font-black text-white uppercase tracking-tighter mb-8 leading-[0.9]">Ready to<br />Get Fabricating?</h2>
              <p className="text-xl text-zinc-400 font-light max-w-xl mx-auto mb-14">Upload your drawings or describe your requirements. We respond within hours.</p>
              <div className="flex flex-col sm:flex-row items-center justify-center gap-6">
                ...
                <a href="tel:+917942957640" className="text-2xl font-display font-black text-white hover:text-primary transition-colors">+91-7942957640</a>
              </div>
            </motion.div>
          </div>
        </section>
