  ...
  return (
    <PageTransition>
      <div className="bg-[#F7F5F1]">
        ...
        {/* HERO */}
        <section className="relative min-h-[70vh] flex items-end overflow-hidden pb-24 bg-[#1C1C1C]">
        ...
        {/* RECENT HIGHLIGHTS */}
        {filter === "All" && (
          <section className="py-16 border-b border-black/8 bg-[#EDEAE4]">
            <div className="container mx-auto px-4">
              ...
              <div className="grid md:grid-cols-3 gap-6">
                {recentPosts.map((post) => (
                  <Link key={post.slug} href={`/blog/${post.slug}`} className="group block">
                    <div className="flex gap-5 items-start p-6 rounded-xl border border-black/8 hover:border-primary/30 hover:bg-[#F7F5F1] transition-all bg-[#EDEAE4]">
        ...
        {/* FILTER BAR */}
        <section className="py-10 border-b border-black/8 sticky top-[72px] bg-[#F7F5F1]/95 backdrop-blur-xl z-40">
          <div className="container mx-auto px-4">
            <div className="flex flex-wrap items-center gap-4">
              <span className="text-xs font-bold tracking-[0.3em] text-slate-400 uppercase hidden sm:block">Filter:</span>
              <div className="flex flex-wrap gap-3">
                {categories.map((cat) => (
                  ...
                  >
                    {cat}
                    <span className={`ml-2 text-[9px] ${filter === cat ? 'text-white/70' : 'text-slate-400'}`}>
        ...
        <section className="py-20 bg-[#F7F5F1]">
          <div className="container mx-auto px-4">
            <motion.div variants={container} initial="hidden" animate="show" key={filter} className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                >
                  <Link href={`/blog/${post.slug}`} className="block relative overflow-hidden aspect-video">
                    ...
                    <div className="absolute top-4 left-4">
                      <span className={`inline-flex px-3 py-1 rounded-full text-[9px] font-bold uppercase tracking-widest border backdrop-blur-sm ${categoryColors[post.category]}`}>{post.category}</span>
                    </div>
                    <div className="absolute bottom-4 right-4 bg-[#F7F5F1]/90 backdrop-blur-sm border border-black/10 rounded-full px-3 py-1">
                      <span className="text-[9px] font-bold text-slate-500 uppercase tracking-widest flex items-center gap-1.5"><Clock className="w-2.5 h-2.5" /> {post.readTime}</span>
                    </div>
                  </Link>
                  <div className="p-7 flex flex-col flex-grow">
                    ...
                    <h3 className="text-xl font-display font-black text-[#1A1A1A] uppercase tracking-tight mb-4 leading-[1.1] group-hover:text-primary transition-colors flex-grow">
                      <Link href={`/blog/${post.slug}`}>{post.title}</Link>
                    </h3>
                    <p className="text-slate-500 text-sm line-clamp-2 mb-6 font-light leading-relaxed">{post.excerpt}</p>
